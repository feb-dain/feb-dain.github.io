...
로렘)깃허브 블로그를 만들었는데 글을 작성해도(커밋을 해도) 잔디가 심어지지 않는 문제 발생. 잔디 심는 재미로 깃허브 블로그를 시작한 것도 있어서 원인과 문제 해결 방법을 찾으려고 열심히 검색을 했다. user.email이 일치하지 않거나 user.name이 일치하지 않으면 이런 문제가 발생할 수 있다고 한다. 하지만 나의 원인은 이게 아니었다. jekyll 테마 깃허브에서 Fork를 눌러 그냥 복사해왔기 때문이었다. Fork한 레파지토리는 아무리 커밋해도 잔디가 심어지지 않는다고…
<br>
